...
func TestNegativeResultRejected(t *testing.T) {
	_, _, err := billingexpr.RunExpr(`tier("bad", -1)`, billingexpr.TokenParams{})
	if err == nil {
		t.Error("expected negative result error")
	}
}
...
const benchComplexExpr = `len <= 200000 ? tier("standard", p * 3 + c * 15 + cr * 0.3 + cc * 3.75 + cc1h * 6 + img * 3 + ai * 10 + ao * 40) : tier("long_context", p * 6 + c * 22.5 + cr * 0.6 + cc * 7.5 + cc1h * 12 + img * 6 + ai * 20 + ao * 80)`
