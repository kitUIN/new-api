...
// RetryParam is request-scoped relay retry state and must not be shared across goroutines.
...
func (p *RetryParam) nextChannelWithRetryExclusions(group string, retry int) (*model.Channel, error) {
	...
	for {
		...
		p.ExcludedChannelIDs = p.ExcludedChannelIDs[1:]
	}
}
...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (selected *model.Channel, selectedGroup string, selectionErr error) {
	...
	if modelCombinationChannel, modelCombinationGroup, enabled, modelCombinationErr := ResolveModelGroupCombinationChannel(param.Ctx, param.ModelName, param.ExcludedChannelIDs); enabled {
		return modelCombinationChannel, modelCombinationGroup, modelCombinationErr
	} else if ratio_setting.IsGroupCombination(param.TokenGroup) {
		...
		channel, err = param.nextChannelWithRetryExclusions(param.TokenGroup, param.GetRetry())
		if err != nil {
			return nil, param.TokenGroup, err
		}
	}
