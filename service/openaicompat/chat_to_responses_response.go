...
const (
	...
	responsesEventOutputTextAnnotation  = "response.output_text.annotation.added"
...
type ChatToResponsesStreamState struct {
	...
	hostedOutputs     []dto.ResponsesOutput
...
// FailChatCompletionsStreamToResponses emits Responses-native terminal errors
// after an SSE response has started.
func FailChatCompletionsStreamToResponses(state *ChatToResponsesStreamState, code, message, param string) []ChatToResponsesStreamEvent {
	if state == nil || state.finalized {
		return nil
	}
	...
	events := make([]ChatToResponsesStreamEvent, 0, 4)
	if !state.sentCreated {
		state.sentCreated = true
		events = append(events, responsesStreamEvent(responsesEventCreated, dto.ResponsesStreamResponse{
			Response: state.createdResponse(),
	...
	events = append(events, state.doneDeltaEvents()...)
	state.finalized = true
	events = append(events, responsesStreamEvent("error", dto.ResponsesStreamResponse{
		Code:    code,
		Message: message,
		Param:   param,
	}))
	...
	events = append(events, responsesStreamEvent(responsesEventFailed, dto.ResponsesStreamResponse{
		Response: response,
	}))
	return events
}

// AppendCompletedWebSearchToResponses appends Gemini's server-side search as
// a complete Responses hosted-tool lifecycle.
func AppendCompletedWebSearchToResponses(state *ChatToResponsesStreamState, queries []string) ([]ChatToResponsesStreamEvent, error) {
	if state == nil {
		return nil, errors.New("chat-to-responses stream state is required")
	}
	if state.finalized || len(queries) == 0 {
		return nil, nil
	}
	action, err := common.Marshal(map[string]any{
		"type":    "search",
		"queries": queries,
	})
	if err != nil {
	...
	outputIndex := state.nextIndex("hosted", len(state.hostedOutputs))
	inProgress := dto.ResponsesOutput{
	...
	state.hostedOutputs = append(state.hostedOutputs, completed)

	events := make([]ChatToResponsesStreamEvent, 0, 6)
	if !state.sentCreated {
		state.sentCreated = true
		events = append(events, responsesStreamEvent(responsesEventCreated, dto.ResponsesStreamResponse{
			Response: state.createdResponse(),
	...
	events = append(events,
		...
		responsesStreamEvent("response.web_search_call.in_progress", dto.ResponsesStreamResponse{
			OutputIndex: intPtr(outputIndex),
			ItemID:      id,
		}),
		responsesStreamEvent("response.web_search_call.searching", dto.ResponsesStreamResponse{
			OutputIndex: intPtr(outputIndex),
			ItemID:      id,
		}),
		responsesStreamEvent("response.web_search_call.completed", dto.ResponsesStreamResponse{
			OutputIndex: intPtr(outputIndex),
			ItemID:      id,
		}),
...
func (s *ChatToResponsesStreamState) appendAnnotationDelta(raw []byte) ([]ChatToResponsesStreamEvent, error) {
	...
	for _, annotation := range annotations {
		annotationJSON, err := common.Marshal(annotation)
		if err != nil {
		...
		events = append(events, responsesStreamEvent(responsesEventOutputTextAnnotation, dto.ResponsesStreamResponse{
			OutputIndex:     intPtr(s.textOutputIndex),
			ContentIndex:    intPtr(0),
...
func (s *ChatToResponsesStreamState) finalResponse() *dto.OpenAIResponsesResponse {
	...
	for _, ref := range s.outputOrder {
		...
		case "hosted":
			if ref.ToolIndex >= 0 && ref.ToolIndex < len(s.hostedOutputs) {
				output = append(output, s.hostedOutputs[ref.ToolIndex])
			}
		}
	}
...
func chatAnnotationsToResponses(raw []byte) ([]interface{}, error) {
	...
	for _, annotation := range annotations {
		if strings.TrimSpace(common.Interface2String(annotation["type"])) != "url_citation" {
...
func chatToolCallToResponsesOutput(toolCall dto.ToolCallRequest, responseID string, index int, status string) (dto.ResponsesOutput, error) {
	...
	return dto.ResponsesOutput{
		...
		Status:    status,
		CallId:    callID,
		Arguments: string(toolCall.Custom),
	}, nil
}
